...
RSpec.describe MyEmail, type: :model do
  it{should validate_presence_of(:email).with_message("Olvidaste poner correo")}
  ...
  it "should not create with invalid email" do
  	email = MyEmail.new(email:"daniel")
  	expect(email.valid?).to be_falsy
  end

  it "should not create with invalid email" do
  	email = MyEmail.new(email:"danielmartinezrc@gmail.com")
